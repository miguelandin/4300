...
void Game::init(const std::string &path)
{
  ...
  while (std::getline(fin, line))
  {
    ...
    {
      iss >> m_eCf.SR >> m_eCf.CR >> m_eCf.SMIN >> m_eCf.SMAX >> m_eCf.OR >> m_eCf.OG >> m_eCf.OB >> m_eCf.OT >>
          m_eCf.VMIN >> m_eCf.VMAX >> m_eCf.L >> m_eCf.SI >> m_eCf.M >> m_eCf.T;
    }
    else if (type == "Bullet")
    {
...
{
  static auto rng = [](auto min, auto max) { return min + (std::fmod(rand(), (1 + max - min))); };
  ...
  if (m_currentFrame - m_lastEnemySpawnTime > m_eCf.SI)
  {
    ...
    do
    {
      ...
      pos = Vec2f(rng(m_eCf.CR, m_wCf.W - m_eCf.CR), rng(m_eCf.CR, m_wCf.H - m_eCf.CR));
      t++;
    } while (isColliding(player()->get<CTransform>().pos, pos, m_pCf.CR * m_eCf.M, m_eCf.CR));
  ...
  if (ImGui::BeginTabBar("TabBar"))
  {
    ...
    if (ImGui::BeginTabItem("Entity Manager"))
    {
      for (auto &[tag, entities] : m_entities.getEntityMap())
      {
        ...
        {
          if (ImGui::BeginTable(tag.c_str(), 4, ImGuiTableFlags_SizingFixedFit))
          {
            for (auto &e : entities)
            {
              ImGui::TableNextRow();
...
{
  static auto updateInput = [](CInput &input, sf::Keyboard::Scancode code, bool isPressed)
  {
    switch (code)
    {
